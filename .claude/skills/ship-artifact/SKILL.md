...
- **summary** ≤ 2 sentences, written for Łukasz (smart non-specialist), no file lists.
...
- Everything else (ticket, branch, changed files, Create PR URL, Linear link, session link, `claude --resume`, git checkout line) is filled in by the script — don't write it.
...
Prints the page path and a ready Linear comment. Render **after** `git push` so the Create PR button is live; before the push it shows a dashed "PR link after push" placeholder (fine for a mid-session preview, not for handover).
...
Only pass the flags for what you actually did. Ticketless branches just need `--url`.
