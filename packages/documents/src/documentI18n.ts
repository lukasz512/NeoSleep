...
// packages/i18n/en.json etc. — the same source of truth the frontends (and
// @neo/email, see its emailI18n.ts) use. Read via fs (not a JS import) so
// this isn't subject to tsc's rootDir check or Node ESM JSON import-attribute
...
/** Backend equivalent of vue-i18n's $t() for generated-document copy — same flat dotted-key namespace as the frontends and @neo/email's emailT(). */
export function documentT(locale: string | null | undefined, key: string, params?: Record<string, string>): string {
