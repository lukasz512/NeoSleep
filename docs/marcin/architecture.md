# Tattoo Spots — Architektura Techniczna
...
- `next-intl` (4.x) — internacjonalizacja PL/EN
- `next-themes` — dark/light mode
- `@sentry/nextjs` — monitoring frontend
...
|--------|------|-------------|
| `@tattoo-spots/shared` | Typy TypeScript, DTO, enums współdzielone między API i frontend | api, web, client-pwa |
| `@tattoo-spots/ui` | Biblioteka komponentów UI (shadcn/ui), transpilowana przez Next.js | web |
| `@tattoo-spots/config` | Wspólne konfiguracje ESLint, TypeScript, Prettier | wszystkie aplikacje |
...
- `transpilePackages: ['@tattoo-spots/shared', '@tattoo-spots/ui']`
- `next-intl` plugin — i18n server-side
- `@sentry/nextjs` — opakowanie całej konfiguracji
- Security headers na wszystkich routach: HSTS, X-Content-Type-Options, X-Frame-Options: DENY, Referrer-Policy
- Optymalizacja obrazów: AVIF + WebP, CDN z `**.tattoo-spots.app`
