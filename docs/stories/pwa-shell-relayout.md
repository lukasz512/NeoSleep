...
- ⚖️ Compliance: no early flags. Accessibility: the "More" tab and the avatar need aria labels, and the expanded grid closes on Escape and exposes aria-expanded on its toggle.
...
- [ ] Mobile: the bottom bar has the first 4 visible nav items + "More". **Revised 2026-09-25 (Łukasz):** "More" expands the bar in place into a grid of *all* modules, with no "More modules" heading and the current module highlighted. The bar items animate to their grid slots, and "More" becomes a Close chevron. It closes on the chevron, a scrim tap, a drag down, Escape, or navigation. With reduced motion, nothing moves.
