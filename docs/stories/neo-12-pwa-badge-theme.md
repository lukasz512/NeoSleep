...
- `apps/pwa/src/layouts/AppLayout.vue`: a small, low-contrast label is fixed to the bottom-right corner of the viewport. It uses theme `on-surface` at 45%, because the in-app background is the plain surface rather than the login gradient. It ignores clicks. On desktop it is fixed to the bottom-right corner of the viewport.
- Mobile (second follow-up, same day): Łukasz wanted it less visible on phones, showing only once you actually scroll to the bottom. On mobile the label is therefore not fixed. It is rendered after the page content, and the content area is at least one visible screen tall (viewport minus app bar and bottom nav). The label starts just below the fold and appears right above the bottom nav once you scroll down. Checked with real local screenshots (iPhone 13 viewport, dashboard/leads/doctors).
- Test: `packages/ui/src/composables/useAppVersionLabel.spec.ts` covers the empty, prod, dev and local cases.

## Follow-up (2026-09-24): version moved into the left menu

The fixed desktop corner overlapped full-height tables (their last row and pagination), and Łukasz asked for a different placement. The in-app version now sits on the grey left drawer, directly under the account button. It is shown only while the menu is expanded on desktop, or opened on mobile. The collapsed desktop rail is too narrow for it, so it is hidden there. The corner label and the mobile end-of-page label are both removed. Checked with real local screenshots: desktop expanded and collapsed, and the iPhone 13 drawer.
