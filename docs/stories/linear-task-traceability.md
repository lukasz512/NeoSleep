...
- ⚖️ Compliance: No new legal exposure from the traceability piece — if anything it helps GDPR/audit posture (clearer record of change provenance). Confirmed with Łukasz 2026-09-16: Notion holds nothing compliance-relevant (no signed docs, no DPA references) — no export/archive step needed before retiring it.
...
- [x] Notion retirement scope: confirmed 2026-09-16 there's nothing valuable stored in Notion, so this is a low-risk documentation/reference cleanup, not a data migration — no ADR needed. Still tracked as a separate follow-on story from this one (see [project_partner_incident_reporting_automation.md](../../.claude/projects/-Users-lukasz512-Documents-Private-NeoSleep/memory/project_partner_incident_reporting_automation.md), whose planned Notion-sync step should target Linear instead).
