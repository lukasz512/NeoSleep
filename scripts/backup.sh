...
#
# Usage:
#   ./scripts/backup.sh                  # local Docker DB (default)
#   DATABASE_URL=<url> ./scripts/backup.sh   # explicit connection string
#
# Output: backups/neosleep_YYYY-MM-DD_HH-MM-SS.dump  (pg_dump custom format)
#
# Restore:
#   pg_restore --clean --if-exists -d "$DATABASE_URL" backups/<filename>.dump
#
# Schedule (cron example — runs daily at 02:00):
#   0 2 * * * /path/to/neo-crm/scripts/backup.sh >> /var/log/neo-backup.log 2>&1
#
# Notes:
