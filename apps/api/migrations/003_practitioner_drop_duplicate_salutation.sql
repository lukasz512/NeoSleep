-- Migration: 003_practitioner_drop_duplicate_salutation.sql
-- Author: Claude (arch decision: consolidate on identities.title, Option A)
-- Date: 2026-07-13
-- Description: practitioner.salutation duplicated identities.title (same concept,
--   two physical columns — a TPT pattern violation). Consolidate on
--   identities.title as the single source of truth for all person types
--   (patient, practitioner, users). Redefines create_tenant_schema() so future
--   tenants don't get the duplicate column, then drops it from every tenant
--   schema that already exists.
--
-- ROLLBACK SQL (run only if migration must be reversed in production):
-- Preconditions: any salutation values entered into practitioner.salutation
--   after this migration ran are NOT recoverable — they were never written
--   there (app code now reads/writes identities.title instead). Re-adding the
--   column just restores an empty field, it does not restore prior data.
--   DO $$
--   DECLARE r RECORD;
--   BEGIN
--     FOR r IN SELECT db_schema FROM platform.tenants LOOP
--       EXECUTE format('ALTER TABLE %I.practitioner ADD COLUMN IF NOT EXISTS salutation TEXT', r.db_schema);
--     END LOOP;
--   END $$;
--   -- Then revert create_tenant_schema() to the version in 001_tenant_schema.sql.
...
-- =============================================================================
-- Drop the duplicate column from every tenant schema that already exists.
-- create_tenant_schema() above no longer creates it, but that only affects
-- schemas created from this point forward.
-- =============================================================================
DO $$
DECLARE
  r RECORD;
BEGIN
  FOR r IN SELECT db_schema FROM platform.tenants LOOP
    EXECUTE format('ALTER TABLE %I.practitioner DROP COLUMN IF EXISTS salutation', r.db_schema);
  END LOOP;
END $$;
