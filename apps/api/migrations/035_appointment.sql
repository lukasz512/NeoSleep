-- =============================================================================
-- Migration 035: appointment — a patient's clinical visit with a doctor
--
-- NEO-27 (docs/ADR-026-appointment-entity.md, docs/stories/calendar-scheduling-epic.md).
-- Not `encounter` (rep → HCP detailing call, no patient) and not `visit_plan`
-- (dead schema) — a new FHIR-inspired Appointment.
--
-- - No double booking: an EXCLUDE constraint (btree_gist) rejects two live
--   (not cancelled, not deleted) appointments of the same practitioner whose
--   [start_at, end_at) ranges overlap. The API maps it (SQLSTATE 23P01) to 409.
-- - territory_id is copied from the patient at booking time, so staff scoping
--   works like every other territory-scoped entity (ltree `<@`).
-- - timezone is the clinic's IANA zone at booking time (Łukasz, 2026-09-26:
--   times are shown in the clinic's zone, not the viewer's).
-- - type has one value ('visit') in v1; more types and durations come later.
-- - location_type 'online' + online_url exist for later; the v1 UI only books
--   in-clinic visits.
-- - treatment_plan_id links an OrthoApnea plan; treatment_plan.appointment_at
--   stays as is (never set by any code path, nothing to migrate) and will be
--   dropped in a later cleanup.
--
-- Tenant-table change → create_tenant_schema() regenerated at the bottom
-- (scripts/generate-create-tenant-schema.ts); CI parity check verifies.
--
-- ROLLBACK (manual):
--   DROP TABLE <schema>.appointment;  (per tenant)
--   and re-run 034's create_tenant_schema() body. btree_gist can stay.
-- =============================================================================

-- Same as 022 for ltree: a plain Postgres (CI, local docker) has no
-- `extensions` schema until something creates it.
...
BEGIN
  FOR r IN SELECT db_schema FROM platform.tenants LOOP
    ...
    EXECUTE format('CREATE TABLE IF NOT EXISTS %I.appointment (
        ...
        sleep_study_id     UUID        REFERENCES %I.sleep_study(id) ON DELETE SET NULL,
        treatment_plan_id  UUID        REFERENCES %I.treatment_plan(id) ON DELETE SET NULL,
        created_by_user_id UUID        NOT NULL REFERENCES %I.users(id) ON DELETE RESTRICT,
        type               TEXT        NOT NULL DEFAULT ''visit'' CHECK (type IN (''visit'')),
        status             TEXT        NOT NULL DEFAULT ''scheduled''
                             CHECK (status IN (''scheduled'', ''completed'', ''cancelled'', ''no_show'')),
        start_at           TIMESTAMPTZ NOT NULL,
        end_at             TIMESTAMPTZ NOT NULL,
        timezone           TEXT        NOT NULL DEFAULT ''UTC'',
        location_type      TEXT        NOT NULL DEFAULT ''clinic'' CHECK (location_type IN (''clinic'', ''online'')),
        online_url         TEXT,
        notes              TEXT,
      ...
      )', r.db_schema, r.db_schema, r.db_schema, r.db_schema, r.db_schema, r.db_schema, r.db_schema, r.db_schema);

    EXECUTE format('ALTER TABLE %I.appointment DROP CONSTRAINT IF EXISTS appointment_no_double_booking', r.db_schema);
    EXECUTE format('ALTER TABLE %I.appointment ADD CONSTRAINT appointment_no_double_booking
        EXCLUDE USING gist (practitioner_id WITH =, tstzrange(start_at, end_at, ''[)'') WITH &&)
        WHERE (deleted_at IS NULL AND status <> ''cancelled'')', r.db_schema);

    EXECUTE format('CREATE INDEX IF NOT EXISTS %I ON %I.appointment (patient_id)', 'appointment_patient_idx', r.db_schema);
    EXECUTE format('CREATE INDEX IF NOT EXISTS %I ON %I.appointment (practitioner_id, start_at)', 'appointment_practitioner_start_idx', r.db_schema);
    EXECUTE format('CREATE INDEX IF NOT EXISTS %I ON %I.appointment (territory_id, start_at)', 'appointment_territory_start_idx', r.db_schema);
    EXECUTE format('CREATE INDEX IF NOT EXISTS %I ON %I.appointment (start_at)', 'appointment_start_idx', r.db_schema);
...
-- with migrations 000-035 (part above) applied. Do not hand-edit; regenerate.
-- -----------------------------------------------------------------------------
...
BEGIN
  ...
  EXECUTE $tenant_ddl$CREATE TABLE IF NOT EXISTS appointment (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    ...
    timezone text DEFAULT 'UTC'::text NOT NULL,
    location_type text DEFAULT 'clinic'::text NOT NULL,
    online_url text,
    notes text,
    ...
    CONSTRAINT appointment_end_after_start CHECK ((end_at > start_at)),
    CONSTRAINT appointment_location_type_check CHECK ((location_type = ANY (ARRAY['clinic'::text, 'online'::text]))),
    CONSTRAINT appointment_status_check CHECK ((status = ANY (ARRAY['scheduled'::text, 'completed'::text, 'cancelled'::text, 'no_show'::text]))),
    CONSTRAINT appointment_type_check CHECK ((type = 'visit'::text))
);$tenant_ddl$;
  ...
  EXECUTE $tenant_ddl$CREATE TABLE IF NOT EXISTS questionnaire_request (
    ...
    CONSTRAINT questionnaire_request_kind_check CHECK ((kind = ANY (ARRAY['medical_history'::text, 'stop_bang'::text, 'bundle'::text])))
);$tenant_ddl$;
...
END) STORED,
    ...
    neck_cm numeric(3,1),
    bmi numeric(3,1),
    CONSTRAINT stop_bang_screening_measurements_check CHECK ((((height_cm IS NULL) OR ((height_cm >= (100)::numeric) AND (height_cm <= (230)::numeric))) AND ((weight_kg IS NULL) OR ((weight_kg >= (25)::numeric) AND (weight_kg <= (350)::numeric))) AND ((neck_cm IS NULL) OR ((neck_cm >= (20)::numeric) AND (neck_cm <= (70)::numeric))) AND ((bmi IS NULL) OR ((bmi >= (5)::numeric) AND (bmi <= 99.9))))),
  ...
  EXECUTE $tenant_ddl$CREATE INDEX IF NOT EXISTS appointment_patient_idx ON appointment USING btree (patient_id);$tenant_ddl$;

  EXECUTE $tenant_ddl$CREATE INDEX IF NOT EXISTS appointment_practitioner_start_idx ON appointment USING btree (practitioner_id, start_at);$tenant_ddl$;

  EXECUTE $tenant_ddl$CREATE INDEX IF NOT EXISTS appointment_start_idx ON appointment USING btree (start_at);$tenant_ddl$;

  EXECUTE $tenant_ddl$CREATE INDEX IF NOT EXISTS appointment_territory_start_idx ON appointment USING btree (territory_id, start_at);$tenant_ddl$;
  ...
  EXECUTE $tenant_ddl$ALTER TABLE appointment DROP CONSTRAINT IF EXISTS appointment_created_by_user_id_fkey;$tenant_ddl$;

  EXECUTE $tenant_ddl$ALTER TABLE appointment DROP CONSTRAINT IF EXISTS appointment_organization_id_fkey;$tenant_ddl$;

  EXECUTE $tenant_ddl$ALTER TABLE appointment DROP CONSTRAINT IF EXISTS appointment_patient_id_fkey;$tenant_ddl$;

  EXECUTE $tenant_ddl$ALTER TABLE appointment DROP CONSTRAINT IF EXISTS appointment_practitioner_id_fkey;$tenant_ddl$;

  EXECUTE $tenant_ddl$ALTER TABLE appointment DROP CONSTRAINT IF EXISTS appointment_sleep_study_id_fkey;$tenant_ddl$;

  EXECUTE $tenant_ddl$ALTER TABLE appointment DROP CONSTRAINT IF EXISTS appointment_territory_id_fkey;$tenant_ddl$;

  EXECUTE $tenant_ddl$ALTER TABLE appointment DROP CONSTRAINT IF EXISTS appointment_treatment_plan_id_fkey;$tenant_ddl$;
  ...
  EXECUTE $tenant_ddl$ALTER TABLE appointment DROP CONSTRAINT IF EXISTS appointment_no_double_booking;$tenant_ddl$;

  EXECUTE $tenant_ddl$ALTER TABLE appointment DROP CONSTRAINT IF EXISTS appointment_pkey;$tenant_ddl$;
  ...
  EXECUTE $tenant_ddl$ALTER TABLE appointment
    ADD CONSTRAINT appointment_no_double_booking EXCLUDE USING gist (practitioner_id WITH =, tstzrange(start_at, end_at, '[)'::text) WITH &&) WHERE (((deleted_at IS NULL) AND (status <> 'cancelled'::text)));$tenant_ddl$;

  EXECUTE $tenant_ddl$ALTER TABLE appointment
    ADD CONSTRAINT appointment_pkey PRIMARY KEY (id);$tenant_ddl$;
  ...
  EXECUTE $tenant_ddl$ALTER TABLE appointment
    ADD CONSTRAINT appointment_created_by_user_id_fkey FOREIGN KEY (created_by_user_id) REFERENCES users(id) ON DELETE RESTRICT;$tenant_ddl$;

  EXECUTE $tenant_ddl$ALTER TABLE appointment
    ADD CONSTRAINT appointment_organization_id_fkey FOREIGN KEY (organization_id) REFERENCES organization(id) ON DELETE SET NULL;$tenant_ddl$;

  EXECUTE $tenant_ddl$ALTER TABLE appointment
    ADD CONSTRAINT appointment_patient_id_fkey FOREIGN KEY (patient_id) REFERENCES patient(id) ON DELETE CASCADE;$tenant_ddl$;

  EXECUTE $tenant_ddl$ALTER TABLE appointment
    ADD CONSTRAINT appointment_practitioner_id_fkey FOREIGN KEY (practitioner_id) REFERENCES practitioner(id) ON DELETE CASCADE;$tenant_ddl$;

  EXECUTE $tenant_ddl$ALTER TABLE appointment
    ADD CONSTRAINT appointment_sleep_study_id_fkey FOREIGN KEY (sleep_study_id) REFERENCES sleep_study(id) ON DELETE SET NULL;$tenant_ddl$;

  EXECUTE $tenant_ddl$ALTER TABLE appointment
    ADD CONSTRAINT appointment_territory_id_fkey FOREIGN KEY (territory_id) REFERENCES territory(id) ON DELETE SET NULL;$tenant_ddl$;

  EXECUTE $tenant_ddl$ALTER TABLE appointment
    ADD CONSTRAINT appointment_treatment_plan_id_fkey FOREIGN KEY (treatment_plan_id) REFERENCES treatment_plan(id) ON DELETE SET NULL;$tenant_ddl$;
