-- =============================================================================
-- Migration 036: questionnaire_request.opened_at — when the patient first
-- opened the self-fill link
--
-- NEO-110. The doctor's QR dialog closes itself as soon as the patient has
-- opened the link (scanned the QR); progress is then followed on the Estudios
-- QR status button (NEO-93) and in the checklist. Set once, by the public
-- GET of the link (commands/questionnaireRequest.ts); NULL = not opened yet.
--
...
--
-- ROLLBACK (manual):
--   ALTER TABLE <schema>.questionnaire_request DROP COLUMN opened_at;  (per tenant)
--   and re-run 035's create_tenant_schema() body.
-- =============================================================================
...
BEGIN
  FOR r IN SELECT db_schema FROM platform.tenants LOOP
    EXECUTE format('ALTER TABLE %I.questionnaire_request ADD COLUMN IF NOT EXISTS opened_at TIMESTAMPTZ', r.db_schema);
  END LOOP;
END $$;
...
-- with migrations 000-036 (part above) applied. Do not hand-edit; regenerate.
-- -----------------------------------------------------------------------------
