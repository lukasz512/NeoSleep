...
--
-- This migration replaces the function body with the exact structure that
-- migrations 000-026 produce — generated by running every migration fresh
-- against an empty database and pg_dump --schema-only'ing the resulting
-- "neosleep" schema (same technique apps/api/scripts/sync-test-schema.ts
-- already uses and validates for the isolated test tenant), then converted
-- to a parameterized plpgsql function. Deliberately generated from a FRESH
-- migrations-only database, not the live Supabase dev DB — the two had
-- drifted apart (the live dev DB has push_subscription.keys as NOT NULL and
-- an organization.org_name_unique_idx index that exist in neither this nor
-- any other migration file, i.e. manual changes made directly against the
-- dev database, never captured in a migration). This function matches what
-- the migration files actually specify, which is what CI's from-scratch
-- Postgres service container builds and validates against — a first
-- attempt generated from the live dev DB instead passed locally but failed
...
--
-- Verified before landing this migration: ran every migration fresh against
-- an empty postgres:15 database (matching CI's service container exactly,
-- via Docker locally), applied the generated function, called it twice
-- against a throwaway schema (idempotency), and pg_dump-diffed that schema
-- against a fresh "neosleep" built the same way — identical (order- and
-- formatting-independent comparison; pg_dump does not guarantee stable
-- object ordering across two independent dumps of even identical schemas).
-- Also confirmed as a negative control that the parity check
-- (scripts/check-tenant-schema-parity.ts) fails when the function is
-- deliberately broken, and passes once restored.
-- =============================================================================
