-- =============================================================================
-- Migration 027: Sync create_tenant_schema() with current schema state
--
-- create_tenant_schema() (001_tenant_schema.sql, last redefined in
-- 003_practitioner_drop_duplicate_salutation.sql) had been stale since: 13+
-- later migrations (004, 005, 006, 008, 009, 010, 012, 015, 017, 018, 020,
-- 021, 022, 023, 024, 025, 026) altered EXISTING tenant schemas via ad-hoc
-- loops over platform.tenants, but never touched this function's body.
-- schema_migrations tracks each migration as applied once, globally — so a
-- NEW tenant provisioned via this function got a schema frozen at migration
-- 003, silently missing everything added since (region columns,
-- lead.institution, notification center, partner_link, territory hierarchy,
-- endo_intake, document versioning, etc).
--
-- Both existing tenants (neosleep, fourseasons) are unaffected — both were
-- registered in platform.tenants early (002_seed.sql), so every later
-- ALTER-loop migration already caught them. This only mattered for a tenant
-- provisioned from today onward — which onboard_new_client()
-- (000_platform.sql, "Called by BFF endpoint POST /api/admin/clients") does
-- by calling this function directly. That endpoint doesn't exist yet, but
-- the DB-side onboarding path is already live, so this was a landmine
-- waiting for it to ship, not a someday problem.
--
-- This migration replaces the function body with the exact current
-- structure of the live "neosleep" schema — generated via pg_dump
-- --schema-only (same technique apps/api/scripts/sync-test-schema.ts already
-- uses and validates for the isolated test tenant), then converted to a
-- parameterized plpgsql function:
--   - SET LOCAL search_path TO %I, public, extensions (matches
--     apps/api/src/db/tenant.ts's withTenant() convention exactly) instead
--     of %I-qualifying every single identifier — unqualified table/index
--     names in the dumped DDL then resolve into the target schema.
--   - CREATE TABLE / CREATE INDEX keep IF NOT EXISTS (idempotent as-is).
--   - The "neosleep_" naming prefix baked into index/constraint names (e.g.
--     neosleep_ai_insight_entity_idx) is stripped — the original function
--     built these dynamically as slug||'_suffix', but a pg_dump snapshot
--     only has neosleep's literal names. Postgres index/constraint names
--     only need to be unique within their own schema, not globally, so a
--     new tenant's indexes are unprefixed from here on (existing tenants'
--     already-created names are untouched — this only affects future
--     CREATE TABLE IF NOT EXISTS / CREATE INDEX IF NOT EXISTS calls, which
--     no-op where a same-named object doesn't exist to compare against).
--   - Every constraint is dropped (IF EXISTS) and re-added, so re-running
--     this function for an already-provisioned tenant is still safe — this
--     matters because onboard_new_client() calls create_tenant_schema()
--     unconditionally, even on an ON CONFLICT DO NOTHING retry. Constraints
--     are dropped in dependency-safe order (foreign keys first, since
--     nothing depends on a FK constraint; then primary/unique keys, which
--     by then have no more dependents) — the naive "drop right before
--     re-adding, in pg_dump's original file order" approach breaks on a
--     real re-run (e.g. conversation_pkey can't drop while a later table's
--     FK still references it).
--
-- Verified before landing this migration: applied the generated function
-- against the real dev DB, called create_tenant_schema() twice against a
-- throwaway schema (idempotency), and pg_dump-diffed that schema against
-- "neosleep" with schema names normalized out — byte-for-byte identical.
-- =============================================================================
...
BEGIN
  ...
  -- Matches apps/api/src/db/tenant.ts's withTenant() convention exactly
  -- (SET LOCAL search_path TO "${slug}", public, extensions) — transaction-
  -- scoped, so it never leaks into whatever the caller (e.g. onboard_new_client())
  -- runs afterward, and includes `extensions` for types like territory.path's
  -- extensions.ltree (already schema-qualified below, but kept for parity
  -- with the runtime path in case a future migration adds an unqualified one).
  ...
  EXECUTE $tenant_ddl$CREATE TABLE IF NOT EXISTS stop_bang_screening (
    ...
    score integer GENERATED ALWAYS AS (((((((((snoring)::integer + (tiredness)::integer) + (observed_apnea)::integer) + (pressure)::integer) + (bmi_over_35)::integer) + (age_over_50)::integer) + (neck_circumference_over_40cm)::integer) + (is_male)::integer)) STORED,
    created_at timestamp with time zone DEFAULT now() NOT NULL
);$tenant_ddl$;
  ...
  EXECUTE $tenant_ddl$CREATE UNIQUE INDEX IF NOT EXISTS org_name_unique_idx ON organization USING btree (lower(name)) WHERE (deleted_at IS NULL);$tenant_ddl$;
  ...
  -- Phase 2: drop constraints before re-adding them (idempotency guard).
  -- FK constraints first, then PK/UNIQUE — see comment above dropStatements.
  ...
  -- Phase 3: (re)add every constraint, in pg_dump's original dependency-safe
  -- order (PK/UNIQUE before the FKs that reference them).
