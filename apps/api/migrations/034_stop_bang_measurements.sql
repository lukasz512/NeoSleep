-- =============================================================================
-- Migration 034: stop_bang_screening measurements
--
-- docs/stories/clinical-questionnaire-capture-redesign.md (Łukasz,
-- 2026-09-26): the specialist no longer answers B and N of STOP-Bang as a
-- bare yes/no. They enter the measurements, and the API derives the answer:
--   height_cm + weight_kg → bmi (kg/m², one decimal) → bmi_over_35
--   neck_cm                                          → neck_circumference_over_40cm
-- The raw values are kept so the printed form and a later review can show
-- what was measured, not only the yes/no it produced. All four columns are
-- nullable: older screenings and screenings where the specialist answered
-- yes/no directly have none.
--
-- Ranges are plausibility guards for typing mistakes (e.g. 1620 cm), not
-- clinical limits; the API validates the same ranges with a clear message.
--
...
--
-- ROLLBACK (manual):
--   ALTER TABLE <schema>.stop_bang_screening
--     DROP COLUMN height_cm, DROP COLUMN weight_kg, DROP COLUMN neck_cm, DROP COLUMN bmi;
--   and re-run 033's create_tenant_schema() body.
--
-- Idempotent: ADD COLUMN IF NOT EXISTS; constraints dropped IF EXISTS first.
-- =============================================================================
...
BEGIN
  FOR r IN SELECT db_schema FROM platform.tenants LOOP
    EXECUTE format('ALTER TABLE %I.stop_bang_screening
        ...
        ADD COLUMN IF NOT EXISTS neck_cm   NUMERIC(3,1),
        ADD COLUMN IF NOT EXISTS bmi       NUMERIC(3,1)', r.db_schema);
    EXECUTE format('ALTER TABLE %I.stop_bang_screening
        DROP CONSTRAINT IF EXISTS stop_bang_screening_measurements_check', r.db_schema);
    EXECUTE format('ALTER TABLE %I.stop_bang_screening
        ADD CONSTRAINT stop_bang_screening_measurements_check CHECK (
          (height_cm IS NULL OR height_cm BETWEEN 100 AND 230) AND
          (weight_kg IS NULL OR weight_kg BETWEEN 25 AND 350) AND
          (neck_cm   IS NULL OR neck_cm   BETWEEN 20 AND 70) AND
          (bmi       IS NULL OR bmi       BETWEEN 5 AND 99.9)
        )', r.db_schema);
  END LOOP;
END $$;
