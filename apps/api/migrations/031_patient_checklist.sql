-- =============================================================================
-- Migration 031: per-patient Estudios checklist + one QR for everything the
-- patient has to do
--
-- NEO-36 part 2 (docs/stories/neo-36-estudios-checklist-redesign.md, ADR-024).
--
-- 1. platform.document_template_entity_type (ADR-021) gains, for the
--    patient entity type, WHO fills a document and WHERE it sits in the
--    patient's Estudios checklist — configured by an admin in the Documents
--    editor, so a new document needs no code change:
--      fill_mode  consent  — the patient reads and signs it on their phone
--                 patient  — the patient answers it (QR) or staff fill it
--                 doctor   — the doctor fills it in / prints it
--                 external — a result from outside (uploaded)
--      sort_order position within the checklist (lower first)
--    Seeded for today's patient documents in the order Łukasz set
--    (2026-09-25): consent → patient → doctor (polysomnography is a
--    built-in item, always last, not a template). Existing assignments keep
--    any values an admin already set.
--
-- 2. questionnaire_request (migration 030) can now cover several steps with
--    one link: items = ordered checklist keys (template keys), and
--    completed_items grows as the patient finishes each step; the link is
--    used up (used_at) once every item is done. kind gains 'bundle'.
--    Existing single-kind requests are backfilled.
--
...
--
-- ROLLBACK (manual):
--   ALTER TABLE platform.document_template_entity_type DROP COLUMN fill_mode, DROP COLUMN sort_order;
--   ALTER TABLE <schema>.questionnaire_request DROP COLUMN items, DROP COLUMN completed_items;
--   (restore the 030 kind CHECK) and re-run 030's create_tenant_schema() body.
-- =============================================================================

ALTER TABLE platform.document_template_entity_type
  ADD COLUMN IF NOT EXISTS fill_mode  TEXT CHECK (fill_mode IN ('consent', 'patient', 'doctor', 'external')),
...
VALUES
  ('informedConsent', 'patient', '00000000-0000-0000-0000-000000000000', 'consent', 10),
  ('medicalHistory',  'patient', '00000000-0000-0000-0000-000000000000', 'patient', 20),
  ('stopBang',        'patient', '00000000-0000-0000-0000-000000000000', 'patient', 30),
  ('oralExam',        'patient', '00000000-0000-0000-0000-000000000000', 'doctor',  40),
  ('historiaEndo',    'patient', '00000000-0000-0000-0000-000000000000', 'doctor',  50)
ON CONFLICT (template_key, entity_type) DO UPDATE
  SET fill_mode  = COALESCE(platform.document_template_entity_type.fill_mode, EXCLUDED.fill_mode),
      sort_order = COALESCE(platform.document_template_entity_type.sort_order, EXCLUDED.sort_order);
...
BEGIN
  FOR r IN SELECT db_schema FROM platform.tenants LOOP
    ...
    EXECUTE format('ALTER TABLE %I.questionnaire_request
        ADD COLUMN IF NOT EXISTS items TEXT[] NOT NULL DEFAULT ''{}'',
        ADD COLUMN IF NOT EXISTS completed_items TEXT[] NOT NULL DEFAULT ''{}''', r.db_schema);

    EXECUTE format('ALTER TABLE %I.questionnaire_request DROP CONSTRAINT IF EXISTS questionnaire_request_kind_check', r.db_schema);
    EXECUTE format('ALTER TABLE %I.questionnaire_request ADD CONSTRAINT questionnaire_request_kind_check
        CHECK (kind IN (''medical_history'', ''stop_bang'', ''bundle''))', r.db_schema);

    EXECUTE format('UPDATE %I.questionnaire_request
        SET items = CASE kind WHEN ''medical_history'' THEN ARRAY[''medicalHistory''] WHEN ''stop_bang'' THEN ARRAY[''stopBang''] ELSE items END
        WHERE items = ''{}''', r.db_schema);
    EXECUTE format('UPDATE %I.questionnaire_request SET completed_items = items WHERE used_at IS NOT NULL AND completed_items = ''{}''', r.db_schema);
...
-- with migrations 000-031 (part above) applied. Do not hand-edit; regenerate.
-- -----------------------------------------------------------------------------
