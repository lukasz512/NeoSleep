-- =============================================================================
-- Migration 037: patients may share an email; users, doctors and leads may not
--
-- NEO-111. identities.email was UNIQUE across every identity, so a patient
-- whose email a family member (another patient), a doctor or a user already
-- has could not be saved ("Database error: insertPatient"). Decided by
-- Łukasz 2026-09-26: several patients may share one email (families often use
-- one address); two users must never share one.
--
-- identities is the shared base table (TPT), so a partial unique index needs
-- a marker on identities itself: email_shared = true for patient identities
-- (set by insertPatient; backfilled below). The global UNIQUE becomes a
-- unique index over the non-patient rows only. Login/user lookups already
-- join users, so a patient row with the same email never reaches auth.
--
...
--
-- ROLLBACK (manual, only while no two identities share an email):
--   DROP INDEX <schema>.identities_email_unique_not_shared;
--   ALTER TABLE <schema>.identities ADD CONSTRAINT identities_email_key UNIQUE (email);
--   ALTER TABLE <schema>.identities DROP COLUMN email_shared;  (per tenant)
--   and re-run 036's create_tenant_schema() body.
-- =============================================================================
...
BEGIN
  FOR r IN SELECT db_schema FROM platform.tenants LOOP
    EXECUTE format('ALTER TABLE %I.identities ADD COLUMN IF NOT EXISTS email_shared BOOLEAN NOT NULL DEFAULT false', r.db_schema);
    EXECUTE format('UPDATE %I.identities i SET email_shared = true WHERE EXISTS (SELECT 1 FROM %I.patient p WHERE p.identity_id = i.id)', r.db_schema, r.db_schema);
    EXECUTE format('CREATE UNIQUE INDEX IF NOT EXISTS identities_email_unique_not_shared ON %I.identities (email) WHERE NOT email_shared', r.db_schema);
    EXECUTE format('ALTER TABLE %I.identities DROP CONSTRAINT IF EXISTS identities_email_key', r.db_schema);
  END LOOP;
END $$;
...
-- with migrations 000-037 (part above) applied. Do not hand-edit; regenerate.
-- -----------------------------------------------------------------------------
