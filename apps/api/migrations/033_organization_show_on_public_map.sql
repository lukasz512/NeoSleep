-- =============================================================================
-- Migration 033: organization.show_on_public_map
--
-- NEO-79 (docs/stories/find-specialist-map-refactor.md): the public website's
-- "find a specialist" map (GET /api/v1/public/specialists) listed every
-- active, geocoded organization, including test/demo clinics created while
-- building other features. Łukasz decided (2026-09-25) to add an explicit
-- per-organization flag instead of deleting or deactivating those records:
-- they stay usable inside the CRM, they just never appear on the public map.
--
-- 1. Every tenant's organization table gains
--      show_on_public_map BOOLEAN NOT NULL DEFAULT TRUE
--    Default TRUE keeps every existing real clinic on the map exactly as
--    before. Admins toggle it from the PWA HCO edit form.
--
-- 2. The five known test organizations in the `neosleep` tenant are set to
--    FALSE, by id. Guarded: runs only when the `neosleep` tenant exists in
--    platform.tenants, and the UPDATE matches nothing on databases that do
--    not have those rows (local, CI, other environments) — a no-op there.
--      8d5467ef-f369-4b62-8abe-e1767dbfdf8c  Klinika Testowa
--      56c0973e-40ac-4e21-90a8-8bc5f477620c  Centrum Medyczne Mokotów
--      497eed5a-b380-48c9-86d8-4205c771fe6a  Szpital Kliniczny Warszawa
--      4f76cc8e-2fe9-4c50-9439-38787e432dd8  Klinika Testowa NEO-51
--      74f2f1b6-cab7-4358-96af-611e4579afe6  Clínica de Prueba NEO-51
--
...
--
-- ROLLBACK (manual):
--   ALTER TABLE <schema>.organization DROP COLUMN show_on_public_map;
--   and re-run 031's create_tenant_schema() body.
--
-- Idempotent: ADD COLUMN IF NOT EXISTS; re-running the UPDATE sets the same
-- five rows to FALSE again (an admin who re-enabled one of them in between
-- would see it hidden again only if this migration re-ran, which the
-- migration runner never does for an applied file).
-- =============================================================================
...
BEGIN
  FOR r IN SELECT db_schema FROM platform.tenants LOOP
    EXECUTE format('ALTER TABLE %I.organization
        ADD COLUMN IF NOT EXISTS show_on_public_map BOOLEAN NOT NULL DEFAULT TRUE', r.db_schema);
  END LOOP;

  IF EXISTS (SELECT 1 FROM platform.tenants WHERE slug = 'neosleep' AND db_schema = 'neosleep') THEN
    UPDATE neosleep.organization
       SET show_on_public_map = FALSE,
           updated_at = now()
     WHERE id IN (
       '8d5467ef-f369-4b62-8abe-e1767dbfdf8c',
       '56c0973e-40ac-4e21-90a8-8bc5f477620c',
       '497eed5a-b380-48c9-86d8-4205c771fe6a',
       '4f76cc8e-2fe9-4c50-9439-38787e432dd8',
       '74f2f1b6-cab7-4358-96af-611e4579afe6'
     );
  END IF;
END $$;
...
-- with migrations 000-033 (part above) applied. Do not hand-edit; regenerate.
-- -----------------------------------------------------------------------------
