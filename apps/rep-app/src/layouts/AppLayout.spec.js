...
const APP_MODULE_ROUTES = ["dashboard", "leads", "planner", "hcp", "hco", "presentations"];
...
function getLayoutSource() {
    ...
    const sources = [readFileSync(layoutPath, "utf-8")];
    try {
...
describe("AppLayout", () => {
    ...
    describe("mobile layout (bottom bar and title)", () => {
        ...
        it("mobile header title has symmetric horizontal padding (16px or variable)", () => {
            const source = getLayoutSource();
            const hasMobilePadding = /@media\s*\(\s*max-width[^)]*\)\s*\{[\s\S]*?layout-app__header-title-wrap[\s\S]*?padding-left:\s*(16px|var\(--rep-content-padding-x[^)]*\))/.test(source) ||
    ...
    describe("collapsed sidebar icon alignment", () => {
        it("collapsed sidebar has horizontal padding so icons are not flush to edges", () => {
            const source = getLayoutSource();
            const hasCollapsedPadding = /\.layout-app__sidebar--collapsed\s*\{[\s\S]*?padding:[\s\S]*?(6|8)px/.test(source) ||
        ...
        it("collapsed nav-link centers icon with justify-content center and gap 0", () => {
            const source = getLayoutSource();
            const collapsedNavLink = source.match(/\.layout-app__sidebar--collapsed[\s\S]*?layout-app__nav-link[\s\S]*?\{[\s\S]*?justify-content:\s*center[\s\S]*?gap:\s*0[\s\S]*?\}/);
    ...
    describe("logo and header row (heights and alignment)", () => {
        ...
        it("nav heading has same left margin as nav link padding so MODUŁY aligns with items", () => {
            const source = getLayoutSource();
            expect(source).toMatch(/layout-app__nav-heading[\s\S]*?margin:\s*0\s+0\s+(4|6)px\s+(8|10)px/);
        });
        it("logo has 1px separator line (same as header::after) so both lines match", () => {
            const source = getLayoutSource();
            expect(source.includes("layout-app__logo") &&
                /(::after|&::after)\s*\{[\s\S]*?height:\s*1\s*px/.test(source)).toBe(true);
        });
        it("sidebar is fixed full height (top 0, bottom 0) for consistent layout", () => {
            const source = getLayoutSource();
            expect(source).toMatch(/\.layout-app__sidebar\s*\{[\s\S]*?top:\s*0/);
            expect(source).toMatch(/\.layout-app__sidebar\s*\{[\s\S]*?bottom:\s*0/);
        });
    });
    describe("expanded sidebar: no button-like hover on nav links", () => {
        it("base nav-link hover does not set background (expanded = no button look)", () => {
            ...
            const expandedHoverMatch = source.match(/Expanded: no button-like[\s\S]*?&\s*:hover[\s\S]*?\{([^}]*)\}/);
