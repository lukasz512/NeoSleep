...
import { getPool, getLeadsPaginated, getLeadById, insertLead, updateLead, insertAuditLog } from "../db.js";
...
leadsRouter.get("/api/leads", asyncHandler(async (req, res) => {
...
leadsRouter.get("/api/leads/:id", asyncHandler(async (req, res) => {
...
leadsRouter.post("/api/leads", asyncHandler(async (req, res) => {
    const body = req.body;
...
leadsRouter.patch("/api/leads/:id", asyncHandler(async (req, res) => {
    const id = req.params.id?.trim();
    if (!id) {
        res.status(400).json({ error: "Missing lead id" });
        return;
