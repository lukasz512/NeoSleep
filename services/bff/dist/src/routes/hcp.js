...
import { getPool, getHCPPaginated, getHCPById, insertHCP, updateHCP, insertAuditLog } from "../db.js";
import { asyncHandler } from "../middleware/errorHandler.js";
const DEFAULT_PAGE = 1;
const DEFAULT_LIMIT = 10;
const MAX_LIMIT = 100;
function parseHCPQuery(req) {
...
hcpRouter.get("/api/hcp", asyncHandler(async (req, res) => {
...
hcpRouter.get("/api/hcp/:id", asyncHandler(async (req, res) => {
...
hcpRouter.post("/api/hcp", asyncHandler(async (req, res) => {
    const body = req.body;
...
hcpRouter.patch("/api/hcp/:id", asyncHandler(async (req, res) => {
    const id = req.params.id?.trim();
    if (!id) {
        res.status(400).json({ error: "Missing HCP id" });
        return;
