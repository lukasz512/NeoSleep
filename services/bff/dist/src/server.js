...
/** In dev, allow localhost and LAN IPs (192.168.x.x, 10.x.x.x) on port 5173 for Vite host:true. */
function corsOrigin(origin, cb) {
    ...
    if (process.env.NODE_ENV !== "production") {
        const m = origin.match(/^https?:\/\/(localhost|127\.0\.0\.1|192\.168\.\d+\.\d+|10\.\d+\.\d+\.\d+):(\d+)$/);
        if (m && (m[2] === "5173" || m[2] === "5174")) {
            cb(null, origin);
            return;
        }
    }
